...
mod tests {
    ...
    fn get_context_with_epoch_height(
        ...
        VMContext {
            current_account_id: "bakansm_near".to_string(),
            signer_account_id: "baokhanhdeptrai_near".to_string(),
    ...
    fn test_nonvalidator_cannot_vote() {
        ...
        let validators = HashMap::from_iter(
            vec![
                ("bakansm_near".to_string(), 100),
                ("baokhanhdeptrai_near".to_string(), 100),
            ]
            .into_iter(),
        );
    ...
    fn test_vote_again_after_voting_ends() {
        let context = get_context("bakansm.near".to_string());
        let validators = HashMap::from_iter(vec![("bakansm.near".to_string(), 100)].into_iter());
        testing_env!(context, Default::default(), Default::default(), validators);
        let mut contract = VotingContract::new();
        contract.vote(true);
